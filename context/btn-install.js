class PwaInstaller {
    ...
    addEventListeners() {
        window.addEventListener("beforeinstallprompt", (t) =>
            this.handleBeforeInstallPrompt(t)
        ),
            window.addEventListener("appinstalled", () => this.handleAppInstalled()),
            this.installBtn.addEventListener("click", () =>
                this.handleInstallButtonClick()
            ),
            this.isIOS && this.setupIosPopupHandlers(),
            window.addEventListener("scroll", () => this.handleScroll());
    }
    handleScroll() {
        window.scrollY > 50 ?
            this.installBtn.classList.add("scrolled") :
            this.installBtn.classList.remove("scrolled");
    }
    ...
    handleInstallButtonClick() {
        this.isIOS ?
            this.iosPopup.classList.add("active") :
            this.deferredPrompt && this.promptNativeInstall();
    }
    async promptNativeInstall() {
        ...
        console.log(`Instalación PWA: ${t}`),
            "accepted" === t &&
            ((this.deferredPrompt = null), this.hideInstallButton());
    }
