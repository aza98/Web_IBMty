...
class YouTubeManager {
    ...
    async checkChannelStatus() {
        try {
            const response = await fetch(
                `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=${this.playlistId}&maxResults=1&key=${this.apiKey}`
            );
            ...
            if (broadcastStatus === 'live') {
                this.setUIMode('LIVE', videoId);
            } else {
                this.setUIMode('OFFLINE', videoId);
            }
        } catch (error) {
    ...
    setUIMode(mode, videoId) {
        ...
        this.updateShareButton(
            isLive ? "Transmisión en vivo IBMty" : "Último mensaje IBMty",
            isLive ? "¡Únete a nuestra transmisión en vivo!" : "Escucha nuestro último mensaje.",
            watchUrl
        );
    }
